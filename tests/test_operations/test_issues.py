...
def test_create_issue_required_params(mock_repo, mock_issue):
    """Test create_issue with only required parameters."""
    with patch("pygithub_mcp_server.common.github.GitHubClient") as mock_client:
        # Setup mock
        mock_client.get_instance.return_value.get_repo.return_value = mock_repo
...
def test_create_issue_all_params(mock_repo, mock_issue, mock_milestone):
...
def test_create_issue_invalid_milestone(mock_repo):
...
def test_update_issue_no_changes(mock_repo, mock_issue):
...
def test_update_issue_all_fields(mock_repo, mock_issue, mock_milestone):
...
def test_list_issues_pagination(mock_repo):
...
def test_list_issues_invalid_state(mock_repo):
...
def test_list_issues_invalid_sort(mock_repo):
...
def test_add_issue_comment(mock_repo, mock_issue, mock_comment):
...
def test_list_issue_comments_pagination(mock_repo, mock_issue):
...
def test_update_issue_comment(mock_repo, mock_issue, mock_comment):
...
def test_delete_issue_comment(mock_repo, mock_issue, mock_comment):
...
def test_add_issue_labels(mock_repo, mock_issue, mock_label):
...
def test_remove_issue_label(mock_repo, mock_issue):
